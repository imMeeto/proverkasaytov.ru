...
describe('pii: маскировка', () => {
  ...
  it('маскирует ФИО с инициалами, не трогая обычные слова', () => {
    expect(maskPii('Директор Иванов И.И. одобрил')).toBe('Директор ФИО одобрил');
    expect(maskPii('Ответственный: И.О. Петров')).toBe('Ответственный: ФИО');
    expect(maskPii('Москва, Красная площадь, 1')).toBe('Москва, Красная площадь, 1'); // не ЛП
  });
